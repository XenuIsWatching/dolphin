...
// The frontend-hosted link bus, as a core sees it.
//
// A COPY, kept byte-identical with the block in libretro-godot's
// LinkInterface.hpp and in mGBA's bundled libretro.h. Externals/libretro-common
// tracks upstream and must not be patched for an extension that is not upstream
// yet; when it lands there, this file goes away and the definitions come from
// <libretro.h> like everything else.
//
// Diff the three with:
//   extract() { awk '/^\/\* -+$/{f=1} f{print} f&&/^};$/{exit}' "$1"; }
